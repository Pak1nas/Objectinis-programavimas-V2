...
Student::Student(const string& v, const string& p, const Vector<double>& nd, double egz)
    : zmogus(v, p), nd_(nd), egz_(egz), vid_(0), med_(0) {}

Student::Student(const Student& other)
    : zmogus(other.vardas_, other.pavarde_), nd_(other.nd_), egz_(other.egz_), vid_(other.vid_), med_(other.med_) {}

Student::Student(Student&& other) noexcept
    : zmogus(other.vardas_, other.pavarde_), nd_(move(other.nd_)), egz_(other.egz_), vid_(other.vid_), med_(other.med_) {}

void Student::printInfo() const {
    cout << pavarde_ << " " << vardas_ << " Vid: " << vid_ << " Med: " << med_ << endl;
}
...
ostream& operator<<(ostream& os, const Student& s) {
    os << left << setw(15) << s.pavarde_
       << left << setw(15) << s.vardas_
       << right << setw(20) << fixed << setprecision(2) << s.vid_
       << right << setw(20) << fixed << setprecision(2) << s.med_;
    return os;
}
...
void spausdinti_lentele(const Vector<Student>& A, int pas) {
    ...
    if (pas == 1 || pas == 3) {
        cout << left << setw(15) << "Pavarde"
             << left << setw(15) << "Vardas"
             << right << setw(15) << "Vidurkis"
             << right << setw(15) << "Mediana" << endl;
        cout << string(70, '-') << endl;
    }

    if (pas == 2 || pas == 3) {
        kiet << left << setw(15) << "Pavarde"
             << left << setw(15) << "Vardas"
             << right << setw(15) << "Vidurkis"
             << right << setw(15) << "Mediana" << endl;
        varg << left << setw(15) << "Pavarde"
             << left << setw(15) << "Vardas"
             << right << setw(15) << "Vidurkis"
             << right << setw(15) << "Mediana" << endl;
    }
